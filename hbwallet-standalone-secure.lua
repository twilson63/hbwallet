...
-- hbwallet standalone - Secure implementation with fallback
-- This version detects if running under Hype and uses appropriate modules

-- Detect if we're running under Hype
local running_under_hype = false
local crypto, httpsig

-- Try to load Hype modules
local ok = pcall(function()
    crypto = require("crypto")
    httpsig = require("httpsig")
    running_under_hype = true
end)

-- JSON implementation (always needed)
local json = {}
...
local function generate_jwk()
    if running_under_hype then
        -- Use Hype's secure crypto
        local jwk = crypto.generate_jwk("RS256")
        if not jwk.kty then jwk.kty = "RSA" end
        return jwk
    else
        -- Without Hype, we can't generate secure keys
        error([[
Cannot generate secure RSA keys without Hype framework.

Please either:
1. Install and run with Hype: hype run hbwallet
2. Install Hype from: https://github.com/twilson63/hype
3. Use the pre-built binaries that include Hype runtime

For security reasons, this tool will not generate weak keys.
]])
    end
end
...
local function get_wallet_address(jwk)
    ...
    if running_under_hype then
        ...
        -- Try using system sha256sum (with proper escaping)
        ...
        local escaped_file = "'" .. tmpfile:gsub("'", "'\\''") .. "'"
        
        local cmd
        if os.execute("which sha256sum >/dev/null 2>&1") == 0 then
            cmd = "sha256sum " .. escaped_file .. " 2>/dev/null | cut -d' ' -f1"
        elseif os.execute("which shasum >/dev/null 2>&1") == 0 then
            cmd = "shasum -a 256 " .. escaped_file .. " 2>/dev/null | cut -d' ' -f1"
        else
            os.remove(tmpfile)
            error("No SHA256 implementation available. Please run with Hype.")
        end
        
        local handle = io.popen(cmd)
        local hash_hex = handle:read("*all"):gsub("%s+", "")
        handle:close()
        os.remove(tmpfile)
        
        -- Convert hex to binary
        local hash_bin = hash_hex:gsub("%x%x", function(hex)
            return string.char(tonumber(hex, 16))
        end)
        
        -- Base64url encode
        local hash_b64 = base64.encode(hash_bin)
        return hash_b64:gsub("+", "-"):gsub("/", "_"):gsub("=+$", "")
    end
end
...
Note: For secure key generation, run with Hype framework:
  hype run hbwallet
...
local function main()
    local ok, result = pcall(function()
        ...
        elseif cmd == "generate" or cmd == nil then
